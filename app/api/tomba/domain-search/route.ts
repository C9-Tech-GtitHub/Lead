...
export async function POST(request: Request) {
  try {
    ...
    console.log(`[Tomba] Searching for emails at domain: ${domain}`);
    ...
    console.log(
      `[Tomba] API response received in ${fetchDuration}ms, status: ${response.status}`,
    );

    if (!response.ok) {
      if (response.status === 429) {
        console.error("[Tomba] Rate limit reached");
        return NextResponse.json(
      ...
      return NextResponse.json(
        {
          error:
            errorData.error?.message || "Failed to fetch emails from Tomba",
        },
    ...
    console.log(
      `[Tomba] Found ${data.data?.emails?.length || 0} emails for ${domain}`,
    );
