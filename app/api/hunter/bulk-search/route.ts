...
export async function POST(request: Request) {
  try {
    ...
    if (!hunterApiKey) {
      return NextResponse.json({ error: 'Hunter.io API key not configured' }, { status: 500 })
    }
    ...
    for (const lead of leads) {
      ...
      try {
        ...
        if (!hunterResponse.ok) {
          ...
          throw new Error(errorData.errors?.[0]?.details || `Hunter.io API error: ${hunterResponse.status}`)
        }
