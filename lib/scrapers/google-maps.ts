...
export async function scrapeGoogleMaps({
  ...
  try {
    ...
    if (!normalizedLocation.toLowerCase().includes('australia') &&
        !normalizedLocation.toLowerCase().includes('aus') &&
        !normalizedLocation.toLowerCase().match(/\b(nsw|vic|qld|sa|wa|tas|nt|act)\b/i)) {
    ...
    if (normalizedLocation !== location) {
      console.log(`[Google Maps Scraper] Location normalized: "${location}" -> "${normalizedLocation}"`);
    }
